...
contract Solve is Script {
  Gate gate = new Gate(bytes32(bytes("foo")), bytes32(bytes("baz")), bytes32(bytes("bar"))); //Gate(0xf5693cb51C9401Ad7923dD39dB783068B64EC973);
  function run() external {
    vm.startBroadcast();
    bytes memory data = hex"1337";
    console.logBytes16(bytes16(data));
    gate.resolve(abi.encodeCall(Gate.unlock, data));
    vm.stopBroadcast();
  }
}
